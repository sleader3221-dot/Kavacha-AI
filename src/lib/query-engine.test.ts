...
describe("answerCopilotQuery", () => {
  it("routes Kannada theft and cyber patrol queries through hotspot, graph, and report engines", () => {
    const result = answerCopilotQuery({ query: DEMO_QUERY_KANNADA, role: "scrb_admin" });
  ...
  it("scopes SHO answers to a smaller set of station-actionable hotspots", () => {
    const admin = answerCopilotQuery({ query: "Where is theft increasing?", role: "scrb_admin" });
    const sho = answerCopilotQuery({ query: "Where is theft increasing?", role: "sho" });

    expect(sho.hotspots.length).toBeLessThanOrEqual(admin.hotspots.length);
    expect(sho.generatedZcql).toContain("station IN");
  });

  it("recognizes Kanglish public-safety query language", () => {
    const result = answerCopilotQuery({
